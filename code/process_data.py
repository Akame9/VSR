...
def process_video(vf, viddata, vidctr, faceCascade):
	...
	#Aathira : Deviding the 75 frames into 0-8,1-9,2-10,... i.e creating something similar to windows with 9 frames each
	for i in np.arange(MARGIN,SAMPLE_LEN+MARGIN):
		viddata[vidctr,:,:,:] = temp_frames[COLORS*int(i-MARGIN):COLORS*int(i+MARGIN),:,:]
		vidctr = vidctr+1
	return vidctr
